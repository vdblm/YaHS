            ...
            var2.name = "var2")
  {
      ...
      if (!is.cat(vec)) {
        ...
        freqs$group1 <-
          replace(x = freqs$group1,
                  list = which(freqs$group1 ==
                                 0),
                  1)
        freqs$group2 <-
          replace(x = freqs$group2,
                  list = which(freqs$group2 ==
                                 0),
                  1)
      }
      else {
    ...
    if (plot) {
      ...
      lbl <-
        paste(
          "correlation =",
          formatC(corr$estimate, digits = 2, format = 'g'),
          "\n p-value = ",
          formatC(corr$p.value, digits = 3, format = 'g')
        )
      data$kind <- 0
      data[up, 'kind'] <- 1
      data[down, 'kind'] <- -1
      g <- ggplot2::ggplot(data, ggplot2::aes(x = var1, y = var2)) +
        ggplot2::geom_jitter(aes(col = factor(kind))) + ggplot2::geom_smooth(method = "lm",
                                                                             se = TRUE,
                                                                             color = "black") +
        ggplot2::labs(x = var1.name, y = var2.name, color = var1.name) +  scale_color_manual(values = color_set) +
        theme_minimal() + ggplot2::theme(plot.title = element_blank(),
                                         text = element_text(family = 'black lato')) + guides(col = F) + ggplot2::annotate(
                                           "text",
                                           x = Inf,
                                           y = Inf,
                                           label = lbl,
    ...
    if (permute > 0) {
      ...
      na <-
        1:permute %>% purrr::map(function(x)
          permute::shuffle(up.down)) %>%
        purrr::map(function(x)
          list(up = x[1:length(up)],
               down = x[(length(up) + 1):length(x)])) %>% purrr::map(function(f)
                 kl.calc(data,
                         f[[1]], f[[2]])) %>% purrr::map(function(x)
                           kl.df <<- rbind(kl.df,
                                           x))
      kls <- 1:dim(kl.df)[2] %>% purrr::map(function(i)
        p.val(kl[i],
              kl.df[, i]))
      df <- data.frame(
        KL = kl,
