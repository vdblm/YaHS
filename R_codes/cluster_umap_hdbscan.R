cluster_umap_hdbscan <-
  function(data, dim, min_pt, n_neighbour, groups) {
    embeddings_clustrable <-
      umap::umap(
        data,
        verbose = TRUE,
        random_state = 42,
    ...
    res <-
      dbscan::hdbscan(embeddings_clustrable$layout, minPts = min_pt)
    
    if (is.null(groups)) {
      groups = as.factor(res$cluster)
    }
    g <- ggplot2::ggplot(data = embeddings,
                         ggplot2::aes(x = embeddings[, 1],
...
theme_Publication <- function(base_size = 14) {
  (
    ...
    + ggplot2::theme(
      plot.title = ggplot2::element_text(size = ggplot2::rel(.5),
                                         hjust = 0.5),
