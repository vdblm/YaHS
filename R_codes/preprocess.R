...
library(ggplot2)
## Loading the data
data <- read_xlsx('./Documents/res/YaHS/D Mining Sample NA.xlsx')

## Deleting variables with no data
data <- data.frame(data[, sapply(data, function(x) sum(is.na(x)) / length(x) < 1)])

## Replacing zeros in 'test' variables with NA. 
# Find test variables
...
## Delete stars in char type variables and convert them to numeric.
...
# change FesharD and FesharS when it's reverse
...
## Set wrong values in tests (values not in the choices) to null
...
## Delete columns with more than 7500 NA values
data <- data.frame(data[, sapply(data, function(x) sum(is.na(x)) < 7500)])

## Delete rows with more than 20 NA values in continuous variables
cont_var_number <- which(colnames(data) == 'test1') - 1
data <- data[apply(data[, 1:cont_var_number], 1, function(x) sum(is.na(x)) < 20), ]

## Preprocess using data_preproc
...
#### WARNING. Dataframe INDICIES ARE USED.
proc_data <- subset(proc_data, select = c(1:33,209, 34:208))
